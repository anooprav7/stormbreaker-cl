...
function getComponents() {
    ...
    const directoriesToCopyFrom = ['animations', 'components'];

    directoriesToCopyFrom.forEach((directory, index) => {
        let folderNames = getDirectories(path.resolve(__dirname, `../build/${directory}`));
        log(folderNames)
        nl(1)
        log('exports')
        ...
        folderNames.forEach(componentName => {
            ...
            const getExports = require(path.resolve(__dirname, `../build/${directory}/${componentName}`));
